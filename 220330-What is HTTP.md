...
- 클라이언트-서버 프로토콜(클라이언트가 서버에 요청을 보내면 서버는 그에 대한 응답을 보내는 클라이언트 서버 구조)
...
- 서버가 다수의 클라이언트와 지속적으로 연결을 유지해야한다면 많은 리소스가 발생하기 때문(트래픽 많고 큰 규모의 서비스를 운영할 경우)
...
웹 상에서의 통신규약, 통신 구조에 해당

tcp/ip를 기반으로 구성



요청과 응답 사이에는 여러 게이트웨이, 라우터, 프록시를 거쳐서 가야함
...
- 캐싱: 주어진 리소스의 복사본을 가지고 있다가 같은 요청이 들어오면 서버까지 요청을 전달하지 않고 해당 리소스를 전달

- 필터링: 비정상 요청, 비정상 사이트 차단 등
- 로드 밸런싱: 서버 부하를 막기 위해 명령을 다르게 줘서 한개의 서비스에서 여럿 러 서버가서로 다른 요청을 처리하도록 전달
- 인증: 특정 리소스 접근을 위한 인증
- 로그기록
...
처리가 됏다고만 할 경우 body가 비어있을 수도 있고

html, json 등 데이터를 담아서 보낼수도 있다.
...
put delete 거의 안쓰고 이제 거의 post로 사용



options

이 url에서 어떤 method가 가능한지



200: 캐시 가능한 응답

301 permanently: 요청한 리소스의 url이 변경되었음을 알리고 아마 url을 보내줄듯, 백엔드에서 미리 처리를 해둬야할듯

304 not modified: 캐시된 응답이랑 동일해

400 bad request: 요청이 잘못됨, 요청에 포함된 data의 오류(타입이 틀렸다든가, 바디에 보내야할 것을 보내지 않았다든가)일 경우가 가장 많음

403 forbidden: 유저가 콘텐츠에 접근할 권한이 x

409: 업로드 충돌

500 internal server error: 백엔드가 제일 싫어하는 에러, 서버에서 요청을 처리하지 못했음

503: 서비스 복구시간, 서버 유지보수 시간, 서버 과부하



http 버전 설정은 어떻게 하는걸까?

user-agent이름은 왜 다 모질라일까?

user-agent를 다른 걸로 변경해서 보낼수 있다..?

빌드하면 robots.txt가 생긴다.

크롤링을 막는 이유는 요청이너무 많아서 서버에 과부하가 걸릴수도 있으니까 디도스랑 비슷한듯?